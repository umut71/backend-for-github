...
export class PremiumController {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Subscribe to premium plan with Google Play verification',
  })
  ...
  async subscribe(@Req() req: any, @Body() dto: SubscribeDto) {
    ...
    if (dto.subscriptionId && dto.purchaseToken) {
      ...
      const verificationResult =
        await googlePlayVerification.verifySubscriptionPurchase(
          dto.subscriptionId,
          dto.purchaseToken,
        );
      ...
      if (!verificationResult.isValid) {
        logger.error(
          `Subscription verification failed: ${verificationResult.error ?? 'Unknown error'}`,
        );
        throw new BadRequestException('Subscription verification failed');
      }

      // Acknowledge the subscription (required by Google Play)
      await googlePlayVerification.acknowledgeSubscription(
        dto.subscriptionId,
        dto.purchaseToken,
      );

      logger.log(
        `Subscription verified successfully, expires: ${verificationResult.expiryTime ? new Date(verificationResult.expiryTime).toISOString() : 'unknown'}`,
      );
  ...
  async getHistory(@Req() req: any) {
    return this.premiumService.getSubscriptionHistory(req.user.id);
  }
}
